...
inputs = [np.zeros((2,3,256,256)), np.zeros((2,73,27))]

bmodel = EngineOV('/data/SadTalker/checkpoints/pi_render.bmodel')
bout = bmodel(inputs)

np.save('bmodel_out.npy',bout)
# pt_model = torch.jit.load('/data/SadTalker/checkpoints/fast_generator_avgpool2d_cpu_2-3-256-256_2-73-27.pt')
# pt_out = pt_model(*[torch.from_numpy(i).double() for i in inputs])

print(np.mean(np.abs(np.load('ptout.npy')-bout)))
